...
1. Check the [supported models](/supported-models) documentation
2. Try using `--trust-remote-code` if the model uses custom code
3. Consider using the [Transformers fallback](/supported-models/extending/transformers-fallback)
4. See [Adding Support for New Models](/supported-models/extending/support-new-models)
...
See [Hyperparameter Tuning](/advanced-features/hyperparameter-tuning) for detailed guidance.
...
4. **Use custom chat template**: See [Custom Chat Template](/references/custom-chat-template)
...
See [Multi-Node Deployment](/references/multi-node-deployment) for details.
...
See [Quantization](/advanced-features/quantization) for supported formats.
...
- [Hyperparameter Tuning](/advanced-features/hyperparameter-tuning) - Performance optimization
- [Server Arguments](/advanced-features/server-arguments) - Command-line options
